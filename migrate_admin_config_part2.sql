...
-- Completar SaaS si ya ejecutaste migrate_admin_config.sql (versión corta, ~31 líneas).
-- Seguro re-ejecutar: usa CREATE OR REPLACE y DROP POLICY IF EXISTS.
-- Ya debés tener: migrate_cierre_15_medios + migrate_multi_tenant + índice admin + gst_is_admin().

-- ---------------------------------------------------------------------------
-- 1. Helper: keys operativas (cierre de caja sin ser admin)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2. Trigger: proteger rol administrador
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3. gst_configs: solo admin escribe config (excepto cierre_medios_used)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4. gst_profiles: signup + admin gestiona empleados
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 5. Terminales: insert en la propia empresa (admin o primera terminal)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 6. Limpiar políticas duplicadas en cierres (opcional, de migrate_cierre_15)
--    migrate_multi_tenant ya agregó gst_tenant_all que cubre lo mismo.
-- ---------------------------------------------------------------------------
DROP POLICY IF EXISTS cierres_select_own_business ON public.gst_cierres_caja;
DROP POLICY IF EXISTS cierres_insert_own_business ON public.gst_cierres_caja;
DROP POLICY IF EXISTS cierres_update_own_business ON public.gst_cierres_caja;
DROP POLICY IF EXISTS cierres_delete_own_business ON public.gst_cierres_caja;

-- ---------------------------------------------------------------------------
-- 7. Migrar flags "used" embebidos → cierre_medios_used
-- ---------------------------------------------------------------------------
